...
exports.getAdEstimate = async (req, res) => {
  ...
  try {
    const response = await axios.get(
      `https://graph.facebook.com/v22.0/${adAccountId}/reachestimate`,
      {
        params: {
          access_token: accessToken,
...
Respond in JSON format:
{
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (err?.response?.status === 429 && attempt < 3) {
        console.log(
          `Gemini rate limited, retrying in 5s (attempt ${attempt})...`,
        );
  ...
  if (!content) {
    console.error(
      "Gemini response content is empty or undefined.",
      geminiResponse.data,
    );
